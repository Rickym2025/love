...
export default function ScratchCard({
  ...
  const [isScratched, setIsSubmitted] = useState(false);

  // Render per la Landing Page (revealText + subText)
  if (revealText) {
    return (
      <div 
        onClick={() => setIsSubmitted(true)}
        className="relative w-full max-w-md mx-auto h-32 bg-[#FAF7F2] rounded-2xl border-2 border-[#D4AF37]/40 shadow-inner flex flex-col items-center justify-center p-4 text-center select-none cursor-pointer overflow-hidden group"
      >
        <div className={`transition-opacity duration-500 ${isScratched ? "opacity-100" : "opacity-20 blur-sm"}`}>
          <span className="font-serif text-xl font-bold text-[#1E293B]">{revealText}</span>
          {subText && <span className="text-xs text-slate-500 font-semibold block mt-1">{subText}</span>}
        </div>

        {!isScratched && (
          <div className="absolute inset-0 bg-[#D4AF37]/90 flex items-center justify-center text-slate-900 font-bold text-xs uppercase tracking-widest group-hover:bg-amber-400 transition">
