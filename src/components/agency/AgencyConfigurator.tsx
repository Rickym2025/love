...
export default function AgencyConfigurator(props: any) {
  ...
  const sampleCreatedInvitations = [
    {
      ...
      template: "Template A (Arco Romano)",
      slug: "elena-e-davide",
      status: "Attivo",
      rsvpCount: 84,
    },
    {
      ...
      template: "Template B (Cielo & Nuvole)",
    ...
    {
      id: "4",
      couple: "Sofia & Lorenzo",
      date: "22 Giugno 2027",
      location: "Villa Borromeo",
      template: "Template B (Cielo & Nuvole)",
      slug: "sofia-e-lorenzo",
      status: "Attivo",
      rsvpCount: 96,
    },
    {
      id: "5",
      couple: "Chiara & Alessandro",
      date: "14 Luglio 2027",
      location: "Relais Villa Giulia",
      template: "Template A (Arco Romano)",
      slug: "chiara-e-alessandro",
      status: "Attivo",
      rsvpCount: 62,
    },
  ];
  ...
  return (
    <div style={style} className="p-6 md:p-8 space-y-6 text-[#1E293B] w-full max-w-3xl mx-auto">
      ...
      {/* TAB 2: INVITI GIÀ CREATI (LISTA MATRIMONI ATTIVI DELL'AGENZIA) */}
      {activeTab === "list" && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <div key={item.id} className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                  <Link
                    href={`/${item.slug}`}
                    target="_blank"
                    className="p-2 bg-[#FAF7F2] text-[#1E293B] hover:text-[#B8860B] rounded-xl border border-slate-200 text-xs font-bold flex items-center gap-1 transition-all"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => alert(`Download Excel Catering per ${item.couple} avviato!`)}
                    className="p-2 bg-[#1E293B] text-white hover:bg-slate-800 rounded-xl text-xs font-bold flex items-center gap-1 transition-all"
